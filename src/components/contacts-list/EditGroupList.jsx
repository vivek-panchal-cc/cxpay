...
const EditGroupList = (props) => {
    const { data,groupId, selectedItems,fullWidth = false } = props;
    ...
    const memberDeleteConfirmation = (memberAccountNumber,memberMobileNumber) =>{
    ...
    const deleteMemberWithGroup = async (memberAccountNumber) => {
        setIsLoading(true);
        try {
            const { data } = await apiRequest.deleteGroupMember({ group_id: groupId, member_account_number : [memberAccountNumber] });
            if (!data.success) throw data.message;
            var contactData = contactsList.filter(obj => obj.member_account_number != memberAccountNumber);
    ...
    const deleteMember = async (memberAccountNumber,memberMobileNumber) => {
        if(memberAccountNumber != "undefined"){
            ...
            var contactData = contactsList.filter(obj => obj.member_mobile_number != memberMobileNumber);
            setContactsList(contactData);
            setShowDeletePopup(false);
        }
    }
        ...
        <div className="edit-group-slider-main">
            ...
            >
                    ...
                    {contactsList?.map((elm,i) => (
                        <SwiperSlide key={"swiper" + i}>
                            <div className="eg-wrap-main" >
                                <a href="#" className="eg-close-btn" onClick={e => memberDeleteConfirmation(`${elm.member_account_number}`,`${elm.member_mobile_number}`)}>
                                    <img src="/assets/images/cross-red.svg"  alt=""/></a>
                                    <div className="grp-img-wrap">
                                        <img src={elm.member_profile_image ? elm.member_profile_image : '/assets/images/profile-default.svg'} style={elm.member_profile_image ? {} : {objectFit : "none"}} alt=""/>
                                    </div>
                                <p>{elm.member_name}</p>
                            </div>
                        </SwiperSlide>
                ...
                <div className="eg-add-con-wrap">
                    <a href="#" onClick={() => showAddContactPopupData() }>+ Add Contact</a>
                </div>
            </Swiper>
            <ModalConfirmation
